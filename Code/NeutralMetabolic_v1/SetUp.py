...
def initialise_community(nrows, ncols, b_density, M, c, x, species_richness=1, max_diversity=False):
	"""
	Sets up:
...
#~ community = initialise_community(30, 30, b_density, 10**3, c, x)#, species_richness=1, max_diversity=False
...
#~ nested_dispersal_map_birth, nested_dispersal_map = make_nested_dispersal_map((0, 0), shape,
	#~ M = 1000, # kg?
	#~ T = temps,
...
def make_dispersal_map(shape, M, T, B_0, alpha, E, max_revolutions, x, c, birth_map):
	"""Makes a 2D array.
	...
	for i in range(landscape_size):
		dispersal_map_birth[i], dispersal_map[i] = make_nested_dispersal_map(get_xy(i, shape[1]), shape, M, T, B_0, alpha, E, max_revolutions, x, c, birth_map)
